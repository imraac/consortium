...
// // //   return (
// // //     <div className="fixed top-4 right-4 shadow-lg mx-4 my-2 p-3 bg-gradient-to-r from-[#ff7f50] to-[#ff6347] rounded-md z-50 transition-transform duration-2000 ease-in-out transform scale-100 hover:scale-105">
...
// // //     e.preventDefault();
// // //     setError("");
// // //     setSuccess("You've successfully logged in! Please wait account being created"); // Show initial message
// // //     setLoading(true);
...
// // //       // Set a timer to simulate the spinner for 10 seconds
// // //       setTimeout(() => {
...
// // //           setSuccess("You've successfully logged in! Please wait account being created");
...
// // //           }
// // //         }
// // //       }, 5000); // Set spinner timeout for 10 seconds
...
// <div className="flex flex-col-reverse md:flex-row max-w-5xl mx-auto mt-[164px] sm:mt-20 w-full items-center">
...
const Login = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState(""); // State for success message
  const [marginTop, setMarginTop] = useState("mt-[164px]"); // State for dynamic margin-top
  const { login } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    // Function to update margin-top based on screen size
    const handleResize = () => {
      if (window.innerWidth < 640) { // Mobile view (smaller than 640px)
        setMarginTop("mt-20"); // Set smaller margin-top for mobile
      } else {
        setMarginTop("mt-[164px]"); // Keep the larger margin-top for larger screens
      }
    };
  ...
  return (
    <section className="bg-white flex flex-col justify-between">
      ...
      <div className={`flex flex-col-reverse md:flex-row max-w-5xl mx-auto ${marginTop} w-full items-center`}>
        {/* Image Section */}
        <div className="w-full md:w-1/2">
          <img className="rounded-2xl mx-auto mt-8 md:mt-0" src="/test2.svg" alt="Login illustration" />
        </div>
      ...
      <Link 
  to="/signup" 
  className="absolute top-2 left-2 sm:top-4 sm:left-4 md:top-10 md:left-6 flex items-center justify-center w-12 h-12 bg-gradient-to-r from-[#65cafd] via-[#65cafd] to-[#65cafd] text-white rounded-full shadow-lg hover:scale-110 hover:shadow-2xl hover:shadow-[#65cafd] transition-all duration-300"
>
  <FaArrowLeft className="text-xl" />
</Link>
